...
const ItemCount = ({stock, initial, fn, showCounter}) => {
    ...
    useEffect(() => {
        if(stock == 0){
            ...
            setStockMsg(`Just ${stock} available!!!`);
        }
    }, [stock])
        ...
        <>
            {showCounter ? 
                <div>
                    ...
                    <div className="card-actions justify-center">
                        <button onClick={() => {fn(count)}} className="btn btn-outline gap-2" disabled={ableBtnSub}>Add to Cart</button>
                    </div>
                </div>
            :
                <div>
